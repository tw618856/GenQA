...
from GEN_func.HE_func.HEVC_func.HEVCSetupModel import *
from GEN_func.HE_func.HEVC_func.HEVCTestCases import *
import csv
...
if __name__ == '__main__':
    ...
    # Open the output report as HEVCReportMMDDYYHHMMSS.csv

    rptDateTime = datetime.now()
    rptStrDateTime = rptDateTime.strftime('%m%d%Y%H%M%S')
    rptName = 'HEVCReport' + rptStrDateTime + '.csv'
    with open(rptName, 'w') as csvfile:
        ...
        with open('HEVCInput.csv', newline='') as csv_file:
            ...
            # We will also login to the HE and setup the main HE Version Control test model
            ...
            for row in csv_reader:
                if readCount == 0:
                    ...
                    readCount += 1
                    HEVCSetupModel(em)
                    # Navigate to Version Control from Main Menu
                    em.screen.log()

                    assert em.screen.contains("IEF@PRIM"), "Version Control:  Main Menu not found"
